...
//	    			printf("size: %d\r\n", size);
    ...
    switch(getSn_SR(DATA_SOCK))
    {
    	case SOCK_ESTABLISHED :
    		...
    		{
    			case LIST_CMD:  /* 지정한 경우 파일이나 디렉터리 정보를 반환, 지정하지 않은 경우 현재 작업 디렉터리 정보 반환 */ 
    			case MLSD_CMD:
#if defined(_FTP_DEBUG_)
    				printf("previous size: %d\r\n", size);
#endif
#if defined(F_FILESYSTEM)
//	                  scan_files("0:/", (char *)dbuf, (int *)&size);                  
                    memset(dbuf, 0, strlen((const char *)dbuf));
...
char proc_ftpd(char * buf)
{
	...
	{     
		...
		{            
			case USER_CMD:  /* 인증 사용자 이름 */
			case PASS_CMD:  /* 암호 */
			case QUIT_CMD:  /* 연결 끊기 */
            case OPTS_CMD:  /* 기능 옵셩 선택 */
				break;
...
//					sprintf(ftp.filename, "%s/%s", ftp.workingdir, arg);
		...
		case APPE_CMD : /* 이어서 추가 */
		case STOR_CMD:  /* 데이터 입력 및 서버 쪽 파일로 저장 */
			slen = strlen(arg);
			arg[slen - 1] = 0x00;
			arg[slen - 2] = 0x00;
#if defined(_FTP_DEBUG_)
		...
		case CWD_CMD :
			...
			if(slen > 3)
			{
                ...
                if(arg[0] != '/' && arg[0] != '.')
                {
                    if(strlen(ftp.workingdir) != 1)
                    {
                        f_getcwd(sendbuf, sizeof(ftp.workingdir));
                        strcat(sendbuf, "/");
                        strcat(sendbuf, arg);  
                        strcpy(arg, sendbuf+2);
                    }
                    else
                    {
                        sprintf(sendbuf, "/%s", arg);
                        strcpy(arg, sendbuf);
                    }
                }                                
                ...
                {
                    f_getcwd(sendbuf, sizeof(ftp.workingdir));
                    strcpy(ftp.workingdir, sendbuf+2);
                    printf("%s\r\n", ftp.workingdir);
		...
		case XCWD_CMD:
		case ACCT_CMD:  /* 계정 정보 */
		case XRMD_CMD:  /* 전송 구조 설정 */
		case RMD_CMD:   /* 디렉터리 제거 */
		case STRU_CMD:  /* 전송 구조 설정 */
		case MODE_CMD:  /* 전송 모드 설정 */
		case XMD5_CMD:
        ...
        /* 기능 옵션 선택 */   
        case OPTS_CMD:
            slen = sprintf(sendbuf, "200 Always in UTF8 mode.\r\n");
			send(CTRL_SOCK, (uint8_t *)sendbuf, slen);
            break;
