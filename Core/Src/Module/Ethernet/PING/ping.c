...
static uint16_t RandomID = 0; 
uint8_t ping_reply_received = 0; 
static uint16_t RandomSeqNum = 0;

uint16_t htons(uint16_t);
void wait_1ms(unsigned int);
uint16_t checksum(uint8_t *, uint16_t);

uint8_t ping_auto(uint8_t s, uint16_t pCnt, uint8_t *addr)
{
	int32_t len = 0;
	uint8_t cnt = 0;
    float percent = 0.0f; 
    ...
    for (uint8_t i = 0; i < pCnt ; i++) {
		switch (getSn_SR(0)) {
			...
			case SOCK_IPRAW:                
                ...
                if (i == 0) {                    
                    printf("PING %d.%d.%d.%d %d bytes of data.\r\n", addr[0], addr[1], addr[2], addr[3], BUF_LEN);
                    time = HAL_GetTick();
                }
...
//	        printf(" ID:%x  SeqNum:%x CheckSum:%x\r\n", htons(PingRequest.ID), htons(PingRequest.SeqNum), htons(PingRequest.CheckSum)) ;
        printf("%d bytes from %d.%d.%d.%d: ", BUF_LEN, addr[0], addr[1], addr[2], addr[3]);
	}
...
{	 
	...
	if (data_buf[0] == PING_REPLY) {
		...
		tmp_checksum = ~checksum((uint8_t *)&data_buf, len);
...
//		    	printf("\r\n");
            printf("icmp_seq=%x ttl=%d time=%ld ms \r\n", htons(PingReply.SeqNum), (rlen+6), HAL_GetTick() - startTick);  
		...
		if (tmp_checksum != PingReply.CheckSum) {
			printf("CheckSum is in correct %x shold be %x\r\n", (tmp_checksum), htons(PingReply.CheckSum));
		}
        else {
		    printf("\r\n Checksum is correct\r\n") ;					
		}
